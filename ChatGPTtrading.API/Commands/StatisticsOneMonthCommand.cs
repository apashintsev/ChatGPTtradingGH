...
public class StatisticsOneMonthCommand : TelegramCommand
{
    public override string Name { get; } = "Статистика за месяц";
    public override async Task Execute(Message message, ITelegramBotClient client, IMediator mediator, CancellationToken cancellationToken)
    {
        ...
        var data = await mediator.Send(new GetStatisticsQuery(true));
